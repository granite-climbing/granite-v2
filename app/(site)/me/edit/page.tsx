...
export default async function EditProfilePage() {
  ...
  return <main className="min-h-screen bg-black px-4 text-white"><section className="mx-auto min-h-screen w-full max-w-[390px] px-0 pb-12 pt-[74px]"><h1 className="text-[22px] font-black leading-[1.45] tracking-[-0.01em]">프로필을<br />수정해주세요.</h1><ProfileForm action={updateProfileAction} submitLabel="수정 완료" initialValues={{ nickname: user.instagramId ?? user.displayName, gender: user.gender, heightCm: user.heightCm, apeIndexCm: user.apeIndexCm, weightKg: user.weightKg, topBoulderingGrade: user.topBoulderingGrade, topSportGrade: user.topSportGrade, youtubeUrl: user.youtubeId }} /></section></main>;
}
